...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.xqfov.mongodb.net/?retryWrites=true&w=majority`;
...
client.connect((err) => {
  ...
  app.post("/addPackage", (req, res) => {
    ...
    packages.insertOne(tourPackage).then((result) => {
  ...
  app.get("/packages", (req, res) => {
    packages.find().toArray((err, docs) => {
  ...
  app.get("/getPackage/:id", (req, res) => {
    packages.find({ _id: ObjectId(req.params.id) }).toArray((err, docs) => {
  ...
  app.delete("/removeAdmin/:id", (req, res) => {
    admins.deleteOne({ _id: ObjectId(req.params.id) }).then((result) => {
      res.status(200).send(result.deletedCount > 0);
    });
  });
  ...
  app.post("/addTestimonial", (req, res) => {
    ...
    testimonials.insertOne(testimonial).then((result) => {
      res.status(201).send(result.insertedCount > 0);
    });
  });
  ...
  app.get("/testimonials", (req, res) => {
    testimonials.find(req.query).toArray((err, docs) => {
      res.status(200).json(docs);
    });
  });
  ...
  app.delete("/deleteTestimonial/:id", (req, res) => {
    testimonials.deleteOne({ _id: ObjectId(req.params.id) }).then((result) => {
      res.status(200).send(result.deletedCount > 0);
    });
  });
  ...
  app.get("/bookings", (req, res) => {
    bookings.find(req.query).toArray((err, docs) => {
      res.status(200).json(docs);
    });
  });
  ...
  app.delete("/deleteBooking/:id", (req, res) => {
    bookings.deleteOne({ _id: ObjectId(req.params.id) }).then((result) => {
      res.status(200).send(result.deletedCount > 0);
    });
  });
